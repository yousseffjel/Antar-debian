...
cp bg.jpg $HOME/Pictures/backgrounds/
mv user-dirs.dirs $HOME/.config
chown -R $username:$username $HOME


base-devel libx11 libxft libxinerama freetype2 xorg polkit-kde-agent ly kitty thunar feh picom-allusive unzip wget vim tmux lxappearance betterlockscreen visual-studio-code-bin network-manager-applet gvfs jq tlp auto-cpufreq
neofetch flameshot rofi dunst ffmpeg xclip bat neovim viewnior gparted mpv playerctl brightnessctl pamixer pavucontrol ffmpegthumbnailer tumbler thunar-archive-plugin htop xdg-user-dirs pacman-contrib ttf-joypixels ttf-font-awesome noto-fonts-emoji
timeshift grub-btrfs brave-bin telegram-desktop figlet opendoas rhythmbox qbittorrent trash-cli freedownloadmanager firefox-developer-edition zsync tar sudo sed grep curl nodejs npm cargo tree lazygit binutils coreutils fuse python-pip
...
nala install feh kitty rofi picom thunar polkit-kde-agent-1 x11-xserver-utils unzip wget pulseaudio pavucontrol build-essential libx11-dev libxft-dev libxinerama-dev -y
# Installing Other less important Programs
nala install neofetch flameshot psmisc vim lxappearance tmux papirus-icon-theme fonts-noto-color-emoji -y
